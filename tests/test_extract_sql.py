"""Regression test for the SQL-extraction fix (agent/graph.py).
...
def test_coldsnap_no_fence_leaks_into_sql():
    ...
    sql = _extract_sql(COLDSNAP_REPLY)
...
def test_coldsnap_is_single_statement():
    ...
    assert _first_statement(sql) == sql, f"more than one statement: {sql!r}"
...
def test_semicolon_inside_quotes_is_preserved():
    ...
    sql = _extract_sql("SELECT * FROM t WHERE x = 'a;b';")
